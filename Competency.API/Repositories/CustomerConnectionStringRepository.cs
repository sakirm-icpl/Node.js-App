...
using Competency.API.APIModel.ThirdPartyIntegration;
...
namespace Competency.API.Repositories
{
    public class CustomerConnectionStringRepository : Repository<CustomerConnectionString>, ICustomerConnectionStringRepository
    {
        ...
        {
            List<APIModel.ThirdPartyIntegration.ConnectionInfo> clientConnectionStrings = null;
            try
            {
                ...
                using (var context = new CourseContext(optionsBuilder.Options))
                {
                    ...
                    clientConnectionStrings = await (from cc in context.CustomerConnectionString
                                                     select new APIModel.ThirdPartyIntegration.ConnectionInfo { ConnectionString = cc.ConnectionString, ClientCode = cc.Code }).AsNoTracking().ToListAsync();
                }
                #endregion
