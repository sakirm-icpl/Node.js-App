        ...
        public async Task<String> GetConnectionStringAsync(string orgcode)
        {
            ...
            try
            {
                ...
                if (cache.IsAdded(Constants.CacheKeyNames.CONNECTION_STRINGS))
                {
                    ...
                    clientConnectionStrings = cache.Get<List<Payment.API.APIModel.ConnectionInfo>>(Constants.CacheKeyNames.CONNECTION_STRINGS);
                }
                else
                {
                    ...
                    using (var context = new UserDbContext(optionsBuilder.Options))
                    {
                        ...
                        clientConnectionStrings = await (from cc in context.CustomerConnectionString
                                                         select new Payment.API.APIModel.ConnectionInfo { ConnectionString = cc.ConnectionString, ClientCode = cc.DatabaseCode }).AsNoTracking().ToListAsync();
                    }
                    #endregion
