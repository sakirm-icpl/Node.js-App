...
namespace CourseApplicability.API.Repositories
{
    public class ApplicabilityGroupTemplateRepository : Repository<ApplicabilityGroupTemplate>, IApplicabilityGroupTemplate
    {
        ...
        public ApplicabilityGroupTemplateRepository(CoursesApplicabilityContext context, IConfiguration configuration, ICustomerConnectionStringRepository customerConnectionRepository) : base(context)
        {
        ...
        public async Task<bool> TemplateDependancy(int TemplateId)
        {
            int Count = await (from c in _db.AccessibilityRule
                               where c.GroupTemplateId == TemplateId && c.IsDeleted == false && c.IsActive == true
                               select new
                               { c.Id }).CountAsync();
            if (Count > 0)
                return true;
