...
using static CourseApplicability.API.Common.EnumHelper;
...
namespace CourseApplicability.API.Repositories
{
    public class CustomerConnectionStringRepository : Repository<CustomerConnectionString>, ICustomerConnectionStringRepository
    {
        ...
        public CustomerConnectionStringRepository(CoursesApplicabilityContext context,
        ...
        public async Task<string> GetConnectionStringByOrgnizationCodeForExternal(string OrgnizationCode)
        {
            ...
            using (var context = new CoursesApplicabilityContext(optionsBuilder.Options))
            {
        ...
        public CoursesApplicabilityContext GetDbContext()
        {
        ...
        public CoursesApplicabilityContext GetDbContext(string ConnetionString)
        {
        ...
        {
            List<CourseApplicability.API.APIModel.ThirdPartyIntegration.ConnectionInfo> clientConnectionStrings = null;
            try
            {
                ...
                using (var context = new CoursesApplicabilityContext(optionsBuilder.Options))
                {
                    ...
                    clientConnectionStrings = await (from cc in context.CustomerConnectionString
                                                     select new CourseApplicability.API.APIModel.ThirdPartyIntegration.ConnectionInfo { ConnectionString = cc.ConnectionString, ClientCode = cc.Code }).AsNoTracking().ToListAsync();
                }
                #endregion
