...
namespace User.API.Repositories
{
    public class UserRepository : Repository<UserMaster>, IUserRepository
    {
       ...
       // private IRolesRepository _rolesRepository;
        ...
        public async Task<int> UpdateUserProfile(APIUpdateUserProfile apiUser, string OrgCode)
        {
            UserMasterDetails UserDetails = await GetUserDetails(apiUser.Id);
            int Id = UserDetails.Id;
            if (OrgCode == "sbil") // For SBIL NOT Editable set privious mobile number
            {
                UserDetails.MobileNumber = Security.Encrypt(UserDetails.MobileNumber);
            }
            else
            {
            ...
            UserDetails.ReportsTo = Security.Encrypt(UserDetails.ReportsTo.ToLower());
            UserDetails.TimeZone = apiUser.TimeZone;
            UserDetails.Currency = apiUser.Currency;
            UserDetails.Id = Id;
            UserDetails.UserMasterId = apiUser.Id;
            UserDetails.ModifiedBy = apiUser.Id;
            UserDetails.ModifiedDate = DateTime.UtcNow;
            this._db.UserMasterDetails.Update(UserDetails);
            await this._db.SaveChangesAsync();
        ...
        public async Task<ApiResponse> GetTeamsCrediential(int UserId)
        {
            ApiResponse obj = new ApiResponse();
            ConfigurableValues configurableValues = await this._db.ConfigurableValues.Where(x => x.ValueCode == "TEAMS").FirstOrDefaultAsync();
            var Query = await (from UserWebinarMasters in this._db.UserWebinarMaster
                               where UserWebinarMasters.isDeleted == 0 && (UserWebinarMasters.isDefault == 0 && UserWebinarMasters.CreatedBy == UserId && UserWebinarMasters.WebinarId == configurableValues.ID)
                               select new { UserWebinarMasters.Id, UserWebinarMasters.Username, UserWebinarMasters.TeamsEmail, UserWebinarMasters.isDefault, UserWebinarMasters.Password }).Distinct().ToListAsync();
            ApiGetZoomCred apiGetZoomCred = new ApiGetZoomCred();
            if (Query.Count == 0)
            {
                return obj;
            }
            ApiTeamsCredential apiTeamsCredential = new ApiTeamsCredential();
            apiTeamsCredential.Id = Query[0].Id;
            apiTeamsCredential.isDefault = Query[0].isDefault;
            apiTeamsCredential.Password = Security.Decrypt(Query[0].Password);
            apiTeamsCredential.Password = Security.EncryptForUI(apiTeamsCredential.Password);
            apiTeamsCredential.TeamsEmail = Query[0].TeamsEmail;
            apiTeamsCredential.Username = Query[0].Username;
            obj.ResponseObject = apiTeamsCredential;
            return obj;
        }
