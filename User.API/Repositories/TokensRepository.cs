...
namespace User.API.Repositories
{
    public class TokensRepository : Repository<Tokens>, ITokensRepository
    {
        ...
        public TokensRepository(UserDbContext context, ICustomerConnectionStringRepository customerConnectionString, IHttpContextAccessor httpContext) : base(context)
        {
            this._db = context;
        ...
        public async Task<int> AddFCMToken(int userId, string fcmToken, bool IsIOS= false)
        {
            try
            {
                using (var dbContext = _customerConnectionString.GetDbContext())
                {
                    ...
                    using (var cmd = connection.CreateCommand())
                    {
                        cmd.CommandText = "AddFCMToken";
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add(new SqlParameter("@userId", SqlDbType.Int) { Value = userId });
                        cmd.Parameters.Add(new SqlParameter("@fcmToken", SqlDbType.VarChar) { Value = fcmToken });
                        cmd.Parameters.Add(new SqlParameter("@IsIOS", SqlDbType.VarChar) { Value = IsIOS });
                        DbDataReader reader = await cmd.ExecuteReaderAsync();
                        reader.Dispose();
                    }
                    connection.Close();
        ...
        public async Task<int> DeleteFCMToken(int userId, string fcmToken)
        {
            try
            {
                using (var dbContext = _customerConnectionString.GetDbContext())
                {
                    ...
                    using (var cmd = connection.CreateCommand())
                    {
                        cmd.CommandText = "DeleteFCMToken";
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add(new SqlParameter("@userId", SqlDbType.Int) { Value = userId });
                        cmd.Parameters.Add(new SqlParameter("@fcmToken", SqlDbType.VarChar) { Value = fcmToken });
                        DbDataReader reader = await cmd.ExecuteReaderAsync();
